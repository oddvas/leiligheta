...
/**
 * Base GatewayImpl. Must be subclassed to do something useful. By default, a
 * NullGatewayQueue is registered as the GatewayQueue. If you require event
 * handling synchronized with the AWT event queue, you can manually register an
 * AWTGatewayQueue.
 * 
 * @author Jesse Peterson
 */
public abstract class GatewayImpl extends Object implements Gateway,
ControlEventDispatcher {
	private GatewayState gatewayState;
	protected GatewayQueue gatewayQueue;

	protected Vector gatewayListeners = new Vector();
	...
	/**
	 * Assign a new gateway state. This method works with waitGatewayState() to
	 * correctly perform blocking while waiting for specified gateway states.
	 */
	 ...
	 * the method to return, as defined for the testGatewayState method. If the
	 * state parameter defines an unreachable state (e.g. ALLOCATED |
	 * DEALLOCATED) an exception is thrown.
	 * <P>
	 * The waitGatewayState method can be called successfully in any Gateway
	 * state.
	 * 
	 ...
	 * Gateway.DEALLOCATING_RESOURCES, Gateway.PAUSED, Gateway.RESUMED. This
	 * method fires a GatewayEvent for valid state transitions.
	 */
	 ...
	 * if the Transmitter queue is empty, irrespective of the pause/resume and
	 * allocation states.
	 * <P>
	 ...
	 * <P>
	 * The testGatewayState method can be called successfully in any Gateway
	 * state.
	 * 
	 * @exception IllegalArgumentException
	 *                if the specified state is unreachable
	...
	public void allocate() throws GatewayException, GatewayStateError {
		...
		if (testGatewayState(Gateway.DEALLOCATING_RESOURCES))
			throw new GatewayStateError(
					"Gateway can not be allocated.  It is currently in the DEALLOCATING_RESOURCES state.");
	...
	public void deallocate() throws GatewayException, GatewayStateError {
		...
		if (testGatewayState(Gateway.ALLOCATING_RESOURCES))
			throw new GatewayStateError(
					"Gateway can not be deallocated.  It is currently in the ALLOCATING_RESOURCES state.");
	...
	public void pause() throws GatewayStateError {
		...
		if (testGatewayState(Gateway.DEALLOCATING_RESOURCES
				| Gateway.DEALLOCATED))
			throw new GatewayStateError(
					"Gateway can not be paused.  It is currently in the DEALLOCATING_RESOURCES or DEALLOCATED state.");
	...
	public void resume() throws GatewayStateError {
		...
		if (testGatewayState(Gateway.DEALLOCATING_RESOURCES
				| Gateway.DEALLOCATED))
			throw new GatewayStateError(
					"Gateway can not be resumed.  It is currently in the DEALLOCATING_RESOURCES or DEALLOCATED state.");
	 ...
	 * 
	 * @param gatewayQueue
	 *            GatewayQueue assigned to handle ControlEvents.
	 */
	...
	/**
	 * Queues ControlEvent on registered GatewayQueue for subsequent dispatch to
	 * registered gateway listeners.
	 */
	 ...
	 * Transmitter and Receiver to handle their specific events. However, this
	 * method must be called to handle any events that they do not.
	 * <P>
	 * This method handles plane GatewayEvent and GatewayErrorEvent X10Events.
	 * 
	 * @param event
	 *            the ControlEvent to be propagated to the listeners
	 */
	@Override
