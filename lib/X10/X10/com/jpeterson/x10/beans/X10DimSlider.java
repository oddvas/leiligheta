...
public class X10DimSlider extends JPanel implements ChangeListener {
	...
	private Vector x10TransmissionListeners = null;
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (!source.getValueIsAdjusting()) {
			...
			if (newValue < dimValue) {
				// dim command
				// sendDim((int)Math.round(((1.0 - ((dimValue - newValue) /
				// 22.0))*100) / 4.5));
				sendDim(22 - newValue);
				dimValue = newValue;
			} else if (newValue > dimValue) {
				// bright command
				// sendBright((int)Math.round(((1.0 - ((newValue - dimValue) /
				// 22.0))*100) / 4.5));
				sendBright(newValue);
				dimValue = newValue;
			}
	 ...
	 * 
	 * @param l
	 *            X10 Transmission listener to add.
	 */
	private void fireX10Event(X10Event event) {
		...
		synchronized (this) {
			if (x10TransmissionListeners != null) {
				v = (Vector) x10TransmissionListeners.clone();
			}
		}
		...
		if (v != null) {
			...
			int cnt = v.size();
			for (int i = 0; i < cnt; i++) {
				X10TransmissionListener client = (X10TransmissionListener) v
						.elementAt(i);
	 ...
	 *            X10 Transmission listener to add.
	 */
	public synchronized void addX10TransmissionListener(
			X10TransmissionListener l) {
		if (x10TransmissionListeners == null) {
			x10TransmissionListeners = new Vector();
		}
	 ...
	 * 
	 * @param l
	 *            X10 Transmission listener to remove.
	 */
	public synchronized void removeX10TransmissionListener(
			X10TransmissionListener l) {
	...
	private void writeObject(ObjectOutputStream stream) throws IOException {
		...
		if (v != null) {
			...
			for (int i = 0; i < cnt; i++) {
				...
				X10TransmissionListener l = (X10TransmissionListener) v
						.elementAt(i);
