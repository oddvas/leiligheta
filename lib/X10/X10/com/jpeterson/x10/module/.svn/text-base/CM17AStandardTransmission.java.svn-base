 ...
 */
public class CM17AStandardTransmission extends Object implements
CM17ATransmissionEvent {
	/**
	 ...
	 * Required for transmitting multiple Dim/Bright commands as each command
	 * represents a 5% change. -jep It looks like it sends 12% per command.
	 */
	...
	public CM17AStandardTransmission(DimEvent event) {
		...
		if (remainder > 0) {
			...
			if (factor >= Math.round((float) (PERCENT / 2.0))) {
				percent += factor;
			} else {
				percent -= factor;
			}
		}
		numberToTransmit = percent / PERCENT;
	}
	 ...
	 * 
	 * @param houseCode
	 *            The character for the house code; 'A' through 'P', upper case,
	 *            inclusive.
	 * @exception IllegalArgumentException
	 *                Thrown if the house code is invalid.
	 */
	protected static byte getHouseByte(char houseCode) {
		if (houseCode < 'A' || houseCode > 'P')
			throw new IllegalArgumentException(
					"House code "
							+ houseCode
							+ " is invalid. House code must be 'A' through 'P', upper case, inclusive.");
	 ...
	 * 
	 * @param device
	 *            The number for the device code; 1 through 16, inclusive.
	 * @exception IllegalArgumentException
	 *                Thrown if the device code is invalid.
	 */
	protected static byte getDeviceByte(int device) {
		if (device < 1 || device > 16)
			throw new IllegalArgumentException("Device " + device
					+ " is invalid. Device must be 1 through 16, inclusive.");
	 ...
	 * 
	 * @param port
	 *            Serial port to send the event to.
	 */
	@Override
