...
public class MacroElement implements Serializable {
	 ...
	 * 
	 * @param brightenFirst
	 *            Set to true if the devices should be brought up to 100% before
	 *            the dim is performed, false if the devices should be dimmed
	 *            from their current brightness.
	 */
	 ...
	 * 
	 * @param device
	 *            Device number for this macro element.
	...
	public void addDevice(int device) {
		if (device < MIN_DEVICE || device > MAX_DEVICE)
			throw new IllegalArgumentException("Device number " + device
					+ " illegal. Device number must be between " + MIN_DEVICE
					+ " and " + MAX_DEVICE + " inclusive.");
		devices[device] = 1;
	}
	 ...
	 * 
	 * @param device
	 *            Device to be removed from this macro element.
	 */
