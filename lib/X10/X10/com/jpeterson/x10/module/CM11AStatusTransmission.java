 ...
 */
public class CM11AStatusTransmission extends Object implements
CM11ATransmissionEvent {
	 ...
	 * 
	 * @param parent
	 *            The CM11A device to retrieve the status of.
	 * @param listener
	 *            CM11AStatusListener to notify when status retrieved. May be
	 *            null.
	 */
	...
	public void transmit(InputStream in, OutputStream out)
		...
		if (in.available() < STATUS_SIZE) {
			...
			if (result == -1)
				throw new EOFException(
						"Unexpected end of stream indicator received while retrieving status.");
			byteValue = (byte) result;

			if (System.getProperty("DEBUG") != null) {
				System.out.println("byte read: 0x" + hex.format(byteValue));
				System.out.println("Second chance bytes available: "
						+ in.available());
			}
			...
			if (byteValue == CM11A.CM11_RECEIVE_EVENT
				...
				System.err
				.println("Breakdown in protocol, consuming all bytes in CM11AStatusTransmission.");
		...
		if (numBytesRead != STATUS_SIZE) {
			System.err
			.println("Invalid status buffer size.  Received "
					+ numBytesRead + " bytes out of " + STATUS_SIZE
					+ " bytes.");
