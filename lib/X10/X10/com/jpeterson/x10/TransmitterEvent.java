 ...
 * <CODE>TransmitterListener</CODE> attached to a <CODE>Transmitter</CODE> using
 * the <CODE>addGatewayListener</CODE> method it inherits from the
 * <CODE>Gateway</CODE> interface.
 * <P>
 * The <CODE>TransmitterEvent</CODE> class extends the <CODE>GatewayEvent</CODE>
 * class. Similarly, the <CODE>TransmitterListener</CODE> interface extends the
 * <CODE>GatewayListener</CODE> interface.
 * <P>
 * <CODE>TransmitterEvent</CODE> extends <CODE>GatewayEvent</CODE> with several
 * events that are specialized for tranmitting. It also inherits several event
 * types from <CODE>GatewayEvent</CODE>: <CODE>GATEWAY_ALLOCATED</CODE>,
 * <CODE>GATEWAY_DEALLOCATED</CODE>, <CODE>GATEWAY_ALLOCATING_RESOURCES</CODE>,
...
public class TransmitterEvent extends ControlEvent {
	...
	/**
	 * The transmit queue of the <CODE>Transmitter</CODE> has emptied and the
	 * <CODE>Transmitter</CODE> has changed to the <CODE>QUEUE_EMPTY</CODE>
	 * state. The queue may become empty because output of all items in the
	 * queue is completed.
	 * 
	 */
	...
	/**
	 * The output queue has changed. This event may indicate a change in state
	 * of the <CODE>Transmitter</CODE> from <CODE>QUEUE_EMPTY</CODE> to
	 * <CODE>QUEUE_NOT_EMPTY</CODE>. The event may also occur in the
	 * <CODE>QUEUE_NOT_EMPTY</CODE> state without changing state.
	 * <P>
	 * The output queue changes when (a) a new item is placed on the queue with
	 * a call to one of the <CODE>speak</CODE> methods or (b) when output of the
	 * top item of the queue is completed (again, without leaving an empty
	 * queue).
	 * <P>
	 ...
	 * <CODE>QUEUE_UPDATED</CODE> event when the top item in the output queue
	 * has changed.
	 */
	protected boolean topOfQueueChanged;

	/**
	 * Construct a <CODE>TransmitterEvent</CODE> with a specified event id and
	 * <CODE>topOfQueueChanged</CODE> flag.
	 * 
	 * @param source
	 *            the <CODE>Transmitter</CODE> that issued the event
	 * @param id
	 *            the identifier for the event type
	 * @param topOfQueueChanged
	 *            true if top item on output queue changed
	 * @param oldGatewayState
	 *            gateway state prior to this event
	 * @param newGatewayState
