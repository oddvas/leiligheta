...
/**
 * This is the main object for the embedded X10 package. It is based on the
 * class <code>com.ibm.tspaces.examples.x10.X10Monitor</code>, an example
 * program included in IBM's TSpaces distribution available from <a
 * href="http://alphaworks.ibm.com">http://alphaworks.ibm.com</a>, as well as
 * the full-featured X10 framework available from <a
 * href="http://www.jpeterson.com">http://www.jpeterson.com</a>.
 * 
 * <code>
 ...
 *  </code>
 * 
 * @author Jesse Peterson <a
 *         href="mailto:jesse@jpeterson.com">(jesse@jpeterson.com)</a>
 */
public class CM11AGateway implements Runnable, SerialPortEventListener,
Transmitter {
	...
	private Vector addressListeners, functionListeners;
	 ...
	 * 
	 * @param portName
	 *            Name of serial port connected to CM11A.
	 */
	private void initPort(String portName) {
		try {
			port = (SerialPort) CommPortIdentifier.getPortIdentifier(portName)
					.open("CM11AGateway", 2000);
	...
	/**
	 * This thread operates as the dispatch thread, dispatching X10Events to the
	 * listeners.
	 */
	@Override
	public void run() {
			...
			while (inputStream.available() > 0) {
				...
				case RESPONSE:
					...
					if (length > buffer.length) {
						// ERROR
						System.out.println("Can not handle message.  Too big: "
								+ length
								+ " bytes.  Maximum allowable message size: "
								+ buffer.length + " bytes."); // DEBUG
						consumeData();
						expected = ANY;
						break;
					}
					...
					try {
						while (index < length) {
							count = inputStream.read(buffer, index, length
									- index);
							index += count;
						}
	...
	private void processResponse(byte[] response, int len) {
		...
		for (int i = 1; i < len; i++) {
			...
			if ((response[0] & 1 << i - 1) != 0) {
				...
				case X10Util.X10_FUNCTION_DIM:
					evt = new DimEvent(this, X10Util.byte2houseCode(hiNibble),
							(int) Unsigned.unsigned(response[++i]), 210);
					break;
				...
				case X10Util.X10_FUNCTION_EXTENDED_CODE:
					evt = new ExtendedCodeEvent(this,
							X10Util.byte2houseCode(hiNibble), response[++i], // data
							response[++i]); // command
					break;
			...
			if (evt != null) {
				// send notification
				// System.out.println("Putting event " + evt +
				// " on event queue..."); // DEBUG
				eventQueue.put(evt);
				// System.out.println("Done putting event " + evt +
				// " on event queue"); // DEBUG
			}
		}
	}
	...
	/**
	 * Issue a command. <BR>
	 * For example, to turn device A1 on: <BR>
	 * <CODE>
	 ...
	 * @param function
	 *            Action to take on the device addressed in the address field.
	 */
	...
	public void setClock() {
		...
		setClock(now.get(Calendar.HOUR_OF_DAY), now.get(Calendar.MINUTE),
				...
				now.get(Calendar.DAY_OF_MONTH), now.get(Calendar.DAY_OF_WEEK),
				'A', false, false, false);
	}
	...
	public void setClock(int hour, int minute, int second, int month, int day,
		...
		/**
		 * // store values this.monitoredHouseCode = houseCode; this.currentDay
		 * = dayOfWeek; this.julianDay = dayOfYear; this.hours = hour;
		 * this.minutes = minute; this.seconds = second; if (clearBatteryTimer)
		 * { batteryUsage = 0; }
		 */
	}
