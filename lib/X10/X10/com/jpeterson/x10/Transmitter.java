...
/**
 * Implementors of this interface are typically some sort of Gateway that bridge
 * a software model with a physical implmentation. For example, the CM11A object
 * is a gateway between a software representation and the physical X10 powerline
 * interface via a serial connection.
 */
public interface Transmitter {
	/**
	 * Bit of state that is set when the output queue of a
	 * <CODE>Transmitter</CODE> is empty. The <CODE>QUEUE_EMPTY</CODE> state is
	 * a sub-state of the <CODE>ALLOCATED</CODE> state. An allocated
	 ...
	 * <CODE>QUEUE_NOT_EMPTY</CODE> state when a method places an object on the
	 * output queue. A <CODE>QUEUE_UPDATED</CODE> event is issued to indicate
	 ...
	 * <CODE>getEngineState</CODE> and <CODE>testEngineState</CODE> methods. To
	 * block a thread until the queue is empty: <BLOCKQUOTE>
	 * 
	 * <PRE>
	 ...
	 * <P>
	 * A <CODE>Transmitter</CODE> enters the <CODE>QUEUE_NOT_EMPTY</CODE> from
	 * the <CODE>QUEUE_EMPTY</CODE> state when the <CODE>x10Transmission</CODE>
	 * method is called to place an object on the output queue. A
	 * <CODE>QUEUE_UPDATED</CODE> event is issued to mark this change in state.
	 * <P>
	...
	/**
	 * Transmit an event. This is typically done by an object that implements a
	 * gateway function to some physical media. For example, the CM11A object is
	 * a gateway between a software model and the physical X10 network via a
	 * serial interface.
	 * 
	 * @param e
	 *            The event to send.
