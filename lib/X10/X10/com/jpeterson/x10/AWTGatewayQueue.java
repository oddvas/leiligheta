...
/**
 * The AWTGatewayQueue uses the AWT event queue to synchronize control events
 * with all other AWT events. This should make a Swing GUI thread safe.
 * <p>
 * The method <code>post</code> is used by the class to submit ControlEvents to
 * the AWTEvent queue. An AWTEventSync object is used to receive the event from
 * the AWTEvent. The actual ControlEvent is contained within the AWTEngineEvent
 * received by AWTEventSync. Therefore, AWTEventSync "cracks" the
 * AWTGatewayEvent, pulling out the ControlEvent and then calls
 * ControlEventDispather's <code>dispatch</code> method, passing in the
 * ControlEvent.
 * 
...
public class AWTGatewayQueue extends GatewayQueue {
	...
	public void post(ControlEvent event) {
		AWTControlEvent awtControlEvent = new AWTControlEvent(awtEventSync,
				event, getDispatcher());
	...
	protected class AWTControlEvent extends AWTEvent {
		 ...
		 *            the control event
		 * @param handler
		 *            GatewayImpl class with the dispatchControlEvent to call
		 */
	...
	protected class AWTEventSync extends Component {
		 ...
		 * CONTROL_EVENT_ID events. Other events are sent on to the superclass.
		 * 
		 * @param e
		 *            the event
		 */
