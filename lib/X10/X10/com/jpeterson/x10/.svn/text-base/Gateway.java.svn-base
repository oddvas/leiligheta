...
public abstract interface Gateway {
	...
	/**
	 * Bit of state that is set when a <CODE>Gateway</CODE> is being allocated -
	 * the transition state between <CODE>DEALLOCATED</CODE> to
	 * <CODE>ALLOCATED</CODE> following a call to the <CODE>allocate</CODE>
	 * method. The <CODE>ALLOCATING_RESOURCES</CODE> state has no sub-states. In
	 * the <CODE>ALLOCATING_RESOURCES</CODE> state, many of the methods of
	 * <CODE>Gateway</CODE>, <CODE>Transmitter</CODE>, and <CODE>Receiver</CODE>
	 * will block unitl the <CODE>Gateway</CODE> reaches the
	 * <CODE>ALLOCATED</CODE> state and the action can be performed.
	 */
	public static final long ALLOCATING_RESOURCES = 0x02;

	/**
	 * Bit of state that is set when a <CODE>Gateway</CODE> is in the allocated
	 * state. A gateway in the <CODE>ALLOCATED</CODE> state has acquired the
	 * resources required for it to carry out its core functions.
	 * <P>
	 * A <CODE>Gateway</CODE> is always created in the <CODE>DEALLOCATED</CODE>
	 * state. It reaches the <CODE>ALLOCATED</CODE> state via the
	 * <CODE>ALLOCATING_RESOURCES</CODE> state with a call to the
	...
	/**
	 * Bit of state that is set when a <CODE>Gateway</CODE> is being deallocated
	 * - the transition state between <CODE>ALLOCATED</CODE> to
	 * <CODE>DEALLOCATED</CODE>. The <CODE>DEALLOCATING_RESOURCES</CODE> state
	 * has no sub-states. In the <CODE>DEALLOCATING_RESOURCE</CODE> state, most
	 * methods of <CODE>Gateway</CODE>, <CODE>Transmitter</CODE> and
	 * <CODE>Receiver</CODE> throw an exception.
	 */
	 ...
	 * <CODE>PAUSED</CODE> and <CODE>RESUMED</CODE> states are sub-states of the
	 ...
	 * <CODE>Gateway</CODE>. The format of the returned <CODE>state</CODE> value
	 * is described above.
	 * <P>
	 * A <CODE>GatewayEvent</CODE> is issued each time the <CODE>Gateway</CODE>
	 * changes state.
	 * <P>
	 * The <CODE>getGatewayState</CODE> method can be called successfully in any
	 * <CODE>Gateway</CODE> state.
	 */
	public long getGatewayState();

	/**
	 * Blocks the calling thread until the <CODE>Gateway</CODE> is in a
	 * specified state. The format of the <CODE>state</CODE> value is described
	 * above.
	 * <P>
	 * All state bits specified in the <CODE>state</CODE> parameter must be set
	 * in order for the method to return, as defined for the
	 * <CODE>testGatewayState</CODE> method. If the <CODE>state</CODE> parameter
	 * defines an unreachable state (e.g. <CODE>ALLOCATED | DEALLOCATED</CODE>)
	 ...
	 * is the <CODE>Transmitter</CODE> queue is empty, irrespective of the
	 * allocation states. <BLOCKQUOTE>
	 * 
	 * <PRE>
	 * if (trans.testGatewayState(Transmitter.QUEUE_EMPTY)) ...
	 * </PRE>
	 * 
	 * </BLOCKQUOTE> The <CODE>testGatewayState</CODE> method is equivalent to:
	 * <BLOCKQUOTE>
	 * 
	 * <PRE>
	 ...
	 * </PRE>
	 * 
	 * </BLOCKQUOTE> The <CODE>testGatewayState</CODE> method can be called
	 * successfully in any <CODE>Gateway</CODE> state.
	 * 
	 * @exception IllegalArgumentException
	 *                - if the specified state is unreachable
	 */
	 ...
	 * successfully the <CODE>ALLOCATED</CODE> bit of the gateway state is set,
	 * and the <CODE>testGatewayState(Gateway.ALLOCATED)</CODE> method returns
	 ...
	 * <UL>
	 * <LI><CODE>PAUSED</CODE> or <CODE>RESUMED</CODE>: the pause state depends
	 * upon the existing state of the gateway. In a multi-app environment, the
	 * pause/resume state of the gateway is shared by all apps.</LI>
	 ...
	 * <CODE>GatewayListeners</CODE> attached to the <CODE>Gateway</CODE> to
	 * indicate state changes. First, as the <CODE>Gateway</CODE> changes from
	 * the <CODE>DEALLOCATED</CODE> to the <CODE>ALLOCATING_RESOURCES</CODE>
	 * state, a <CODE>GATEWAY_ALLOCATING_RESOURCES</CODE> event is issued. As
	 * the allocation process completes, the gateway moves from the
	 ...
	 * <CODE>ALLOCATING_RESOURCES</CODE> or <CODE>ALLOCATED</CODE> states. The
	 * method throws an exception in the <CODE>DEALLOCATING_RESOURCES</CODE>
	 * state.
	 * <P>
	 * If any problems are encountered during the allocation process so that the
	 * gateway cannot be allocated, the gateway returns to the
	 ...
	 * or slow (several 10s of seconds) depending upon a range of factors. Since
	 * the <CODE>allocate</CODE> method does not return until allocation is
	 * completed applications may want to perform allocation in a background
	 * thread and proceed with other activities.
	 * 
	 * @exception GatewayException
	 *                if an allocation error occurred or the gateway is not
	 *                operational.
	 * @exception GatewayStateError
	 *                is called for an engine in the
	 ...
	 * <CODE>allocate</CODE>.
	 * <P>
	 * Gateways need to clean up current activities before being deallocated. A
	 * <CODE>Transmitter</CODE> must be in the <CODE>QUEUE_EMPTY</CODE> state
	 * before being deallocated. If the queue is not empty, any objects on the
	 * transmit queue must be cancelled with appropriate events issued.
	 * <P>
	 * While this method is being processed events are issued to any
	 * <CODE>GatewayListeners</CODE> attached to the <CODE>Gateway</CODE> to
	 * indicate state changes. First, as the <CODE>Gateway</CODE> changes from
	 * the <CODE>ALLOCATED</CODE> to the <CODE>DEALLOCATING_RESOURCES</CODE>
	 * state, a <CODE>GATEWAY_DEALLOCATING_RESOURCES</CODE> event is issued. As
	 * the deallocation process completes, the gateway moves from the
	 * <CODE>DEALLOCATING_RESOURCES</CODE> state to the <CODE>DEALLOCATED</CODE>
	 * state and an <CODE>GATEWAY_DEALLOCATED</CODE> event is issued.
	 * <P>
	 * The <CODE>deallocate</CODE> method should only be called for a
	 * <CODE>Gateway</CODE> in the <CODE>ALLOCATED</CODE> state. The method has
	 * no effect for a <CODE>Gateway</CODE> in either the
	 ...
	 * @exception GatewayStateError
	 *                if called for a gateway in the
	 *                <CODE>ALLOCATING_RESOURCES</CODE> state
	...
	/**
	 * Pause the event transmission for the gateway and put the
	 * <CODE>Gateway</CODE> into the <CODE>PAUSED</CODE> state. Pausing a
	 * gateway pauses the underlying gateway for <I>all applications</I> that
	 * are connected to that gateway. Gateways are typically paused and resumed
	 ...
	 * <CODE>ALLOCATING_RESOURCES</CODE> state, the method blocks (waits) until
	 * the <CODE>ALLOCATED</CODE> state is reached and then operates normally.
	 * An error is thrown when pause is called for a gateway in either the
	 ...
	 * 
	 * @exception GatewayStateError
	 *                if called for a gateway in the <CODE>DEALLOCATED</CODE> or
	 ...
	 * event transmission or reception for a paused gateway. Resuming a gateway
	 * resumes the underlying gateway for <I>all applications</I> that are
	 * connected to that gateway. Gateways are typically paused and resumed by
	 * request from a user.
	 * <P>
	 * The specific pause/resume bejavior of tramistters and receivers is
	 * defined in the documentation for the <CODE>pause</CODE> method.
	 * <P>
	 * When a gateway moves from the <CODE>PAUSED</CODE> state to the
	 * <CODE>RESUMED</CODE> state, a <CODE>GATEWAY_RESUMED</CODE> event is
	 * issued to each <CODE>GatewayListener</CODE> attached to the
	 ...
	 * <CODE>ALLOCATED</CODE> state. When resume is called for a gateway in the
	 * <CODE>ALLOCATING_RESOURCES</CODE> state, the method blocks (waits) until
	 * the <CODE>ALLOCATED</CODE> state is reached and then operates normally.
	 * An error is thrown when resume is called for a gateway in either the
	 * <CODE>DEALLOCATED</CODE> or <CODE>DEALLOCATING_RESOURCES</CODE> state.
	 * <P>
	 ...
	 * application can attache multiple listeners to a <CODE>Gateway</CODE>. A
	 * single listener can be attached to multiple gateways.
	 * <P>
	 * The <CODE>GatewayListener</CODE> is extended for both transmission and
	 * reception. Typically, a <CODE>ReceptionListener</CODE> is attached to a
	 * <CODE>Receiver</CODE> and a <CODE>TransmissionListener</CODE> is attached
	 * to a <CODE>Transmitter</CODE>.
	 * <P>
	 * A <CODE>GatewayListener</CODE> can be attached or removed in any state of
	 * a <CODE>Gateway</CODE>.
	 * 
	 * @param listener
	 *            the listener that will reveive <CODE>GatewayEvents</CODE>
	 * @see Transmitter
	 */
	 ...
	 * <CODE>GatewayListener</CODE> can be attached or removed in any state of a
	 * <CODE>Gateway</CODE>.
	 * 
	 * @param listener
